...
### 📈 交易策略（大膽模式）
- **RSI + 纏論混合決策**：RSI 40/60 法則 + 纏論多空分數確認
- **神經網路主動決策**：果蠅大腦放電模式直接參與買賣（閘門>=2，放電差>=3Hz）
- **無知識等級限制**：即使纏論知識不足也能大膽交易，從錯誤中學習
- **視覺觸頂/觸底**：價格百分位 <=25% 買入，>=75% 賣出
- **實戰技能觸發**：信心 >=15% 即可執行交易
- **短冷卻快速迭代**：每步僅 20 秒冷卻，快速累積交易經驗
- **0 手續費模擬**：精確計算已實現盈虧
...
### 🛡️ 穩定性機制
- **看門狗（Watchdog）**：監控程序當機自動重啟，記錄重啟次數
...
> **大膽交易模式**：目前預設為大膽模式，果蠅會頻繁交易、從錯誤中學習。RSI 門檻 40/60、冷卻 20 秒、神經網路主動決策、無知識等級限制。
...
3. **神經模擬**：166,700神經元運算 + 纏論輸入
4. **神經解碼決策**：DNp20 左右解碼 + DNpe017 閘門控制
5. **風控與執行**：下單、冷卻 20 秒（大膽模式）
