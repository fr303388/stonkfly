...
### 📈 交易策略（純纏論自動 + 果蠅共同決策）
- **純纏論自動交易（--chan-auto）**：15分K線纏論結構分析，RSI確認，多空分數驗證
- **果蠅共同決策模式**：纏論有明確訊號時纏論主導；纏論觀望時果蠅強烈訊號可主動交易
- **果蠅強烈訊號門檻**：閘門放電≥3次 + 左右腦頻率差≥3Hz
- **交易理由標註決策者**：[纏論主導]、[果蠅主導]、[混合]
- **止盈止損**：+3%/-3% 自動出場
...
> **純纏論自動交易模式**：使用 `--chan-auto` 啟用，纏論15分K線分析為主，果蠅共同決策。每次買入1顆BTC，冷卻60秒，+3%/-3%止盈止損。
...
5. **風控與執行**：下單、冷卻 60 秒（chan-auto模式）
