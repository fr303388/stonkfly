...
#### 核心邏輯
果蠅和纏論**並行決策**，任一方有明確訊號就觸發交易，不需要等待另一方觀望。

#### 決策流程
```
無持倉時：
  果蠅說BUY 或 纏論說BUY → 買入1顆BTC
  都沒說 → HOLD觀察

有持倉時：
  果蠅說SELL 或 纏論說SELL → 賣出全部持倉
  都沒說 → HOLD觀察
```
...
| `[果蠅自由]` | 只有果蠅說 | 果蠅神經網路自主決策 |
| `[纏論參考]` | 只有纏論說 | 纏論技術分析訊號 |
| `[觀察]` | 都沒說 | 等待機會，記錄模仿率 |

#### 風控參數
- **止盈**：已取消，賣出時間點由趨勢、纏論訊號和果蠅自然決策共同判斷
- **止損**：-3%自動賣出（僅防禦，防止極端行情）
- **每次買入**：1顆BTC，市價單，1%緩衝確保完整成交
- **初始資金**：10,000,000 USDT
- **手續費**：0（模擬環境）
- **冷卻時間**：每筆交易後60秒，避免頻繁交易

#### 果蠅學習機制
- **獲利**：刺激 PAM11 獎勵神經元，強化正確決策的突觸連接
- **虧損**：刺激 PPL101 厭惡神經元，弱化錯誤決策的突觸連接
- **模仿率**：記錄果蠅決策與纏論訊號的一致比例，觀察學習進度
