...
### 📈 交易策略（果蠅自由決策 + 纏論參考）
- **果蠅自由決策模式**：果蠅自然決策直接生效，說BUY就買、說SELL就賣，不受纏論限制
- **纏論備援機制**：果蠅觀望(HOLD)時，纏論訊號會自動觸發交易作為備援
- **決策優先級**：果蠅訊號 > 纏論訊號 > HOLD觀察
- **交易理由標註決策者**：[果蠅自由]、[纏論參考]、[觀察]
...
> **果蠅自由決策模式**：使用 `--chan-auto` 啟用，果蠅自然決策優先，纏論作為備援參考。每次買入1顆BTC，冷卻60秒，止盈已取消（由果蠅和纏論判斷賣出時機），-3%止損保護。
